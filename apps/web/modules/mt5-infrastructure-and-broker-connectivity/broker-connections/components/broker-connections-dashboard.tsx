...
export function BrokerConnectionsDashboard() {
  ...
  const selected = data.brokers.find((broker) => broker.id === selectedId) ?? data.brokers[0] ?? null;
  const spreadRisk = selected ? detectSpreadSpikes(data.spreadLogs, selected.id) : { detected: false, persistent: false, multipleSymbols: false, affectedSymbols: [] as string[], severity: "Info" as const };
  const executionRisk = selected ? detectExecutionDegradation(data.executionQuality, selected.id) : { degraded: false, averageExecutionTimeMs: 0, averageSlippagePoints: 0, requoteRate: 0, rejectionRate: 0 };
  ...
  async function command(label: string, path: string, body?: Record<string, unknown>) {
    if (!path) {
      setNotice("No broker is selected for this action yet.");
      return;
    }
  ...
  const actions = [
    ...
    { label: "Test Connection", path: selected ? `/api/mt5/broker-connections/${selected.id}/test` : "", allowed: data.permissions.canTest && Boolean(selected) },
    { label: "Run Broker Diagnostics", path: "/api/mt5/broker-connections/diagnostics", allowed: data.permissions.canDiagnostics && Boolean(selected), body: selected ? { brokerId: selected.id } : undefined },
    { label: "Reconnect Broker", path: selected ? `/api/mt5/broker-connections/${selected.id}/reconnect` : "", allowed: data.permissions.canReconnect && Boolean(selected) }
  ];
    ...
    <section className="sticky top-14 z-20 rounded-2xl border border-slate-200 bg-white/95 shadow-card backdrop-blur">
      ...
      <div className="flex flex-col gap-5 p-5 xl:flex-row xl:justify-between">
        <div>
          ...
          <p className="mt-3 text-xs text-slate-500">Mode: {data.meta.monitoringMode} | Role: {data.permissions.role} | Selected broker: {selected?.brokerName ?? "None"} | Updated: {time(data.meta.timestamp)}</p>
        </div>
