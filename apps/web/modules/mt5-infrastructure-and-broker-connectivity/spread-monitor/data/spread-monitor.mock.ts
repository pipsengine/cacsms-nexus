import { normalizeSymbol } from "../../mt5-control-center/algorithms/mt5-control-center.algorithms";
import type { SpreadAssetClass, SpreadAlert, SpreadLogEntry, SpreadSnapshot, SpreadThreshold, SpreadTrendPoint, SpreadWorkflowNode } from "../types/spread-monitor.types";
import { calculateSpreadPips } from "../algorithms/spread-monitor.algorithms";
...
export function createMockWorkflow(spreads: SpreadSnapshot[], alerts: SpreadAlert[]): SpreadWorkflowNode[] {
  ...
  const nodes: SpreadWorkflowNode[] = [
    { title: "Tick Received", status: "Healthy", symbolCount: totalSymbols, failedCount: 0, averageDelayMs: 32, latestAlert: "—", aiRecommendation: "Continue monitoring tick freshness." },
    { title: "Spread Calculated", status: "Healthy", symbolCount: totalSymbols, failedCount: 0, averageDelayMs: 18, latestAlert: "—", aiRecommendation: "Track outliers by asset class." },
    ...
    { title: "Peer Broker Compared", status: "Healthy", symbolCount: totalSymbols, failedCount: 0, averageDelayMs: 72, latestAlert: "—", aiRecommendation: "Route to lowest-spread broker when divergence is material." },
    ...
    { title: "Unsafe Trades Blocked", status: critical > 0 ? "Critical" : "Healthy", symbolCount: totalSymbols, failedCount: critical, averageDelayMs: 26, latestAlert: critical ? "Execution blocked on 1+ symbols" : "—", aiRecommendation: "Keep blocked until spreads normalize below warning." },
    { title: "Alert Logged", status: "Healthy", symbolCount: totalSymbols, failedCount: 0, averageDelayMs: 12, latestAlert: alerts[0]?.alertType ?? "—", aiRecommendation: "Ensure audit trail covers threshold + block actions." }
  ];
  return nodes;
}
