...
export function EaBridgeDashboard() {
  ...
  const [selectedId, setSelectedId] = useState("ea-fra-03");
  const [expandedId, updateExpandedId] = useState<string | null>("ea-fra-03");
  ...
  const selected = data.instances.find((instance) => instance.id === selectedId) ?? data.instances[0];
  ...
  const actions = [
    ...
    { label: "Restart Bridge", icon: RotateCcw, path: `/api/mt5/ea-bridge/instances/${selected.id}/restart`, allowed: data.permissions.canRestart },
    { label: "Rotate Bridge Token", icon: KeyRound, path: `/api/mt5/ea-bridge/instances/${selected.id}/rotate-token`, allowed: data.permissions.canRotateToken }
  ];
      ...
      <section className="sticky top-14 z-20 rounded-2xl border border-slate-200 bg-white/95 shadow-card backdrop-blur">
        ...
        <div className="flex flex-col gap-5 p-5 xl:flex-row xl:justify-between">
          <div>
            ...
            <p className="mt-3 text-xs text-slate-500">Mode: {data.meta.monitoringMode} | Role: {data.permissions.role} | Selected EA: {selected.eaName} | Updated: {time(data.meta.timestamp)}</p>
          </div>
