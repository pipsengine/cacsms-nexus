...
import { Activity, AlertTriangle, Bot, Download, Globe2, Menu, RefreshCw, Search, Star, TrendingUp, Waves } from "lucide-react";
...
export function MarketWatchDashboard() {
  ...
  const [selectedId, setSelectedId] = useState("mw-nas100");
  ...
  const selectedStatus = quoteStatus(selected);
  const selectedMove = dailyMovePercent(selected);
  const selectedSpread = spreadPoints(selected);
  ...
  async function command(label: string, path: string) {
    if (!window.confirm(`Confirm ${label.toLowerCase()}? This market-watch operation will be audit-logged.`)) return;
    setNotice(null);
    try {
    ...
    <section className="sticky top-14 z-20 rounded-2xl border border-slate-200 bg-white/95 shadow-card backdrop-blur">
      ...
      <div className="flex flex-col gap-5 p-5 xl:flex-row xl:justify-between">
        <div>
          ...
          <p className="mt-3 text-xs text-slate-500">Mode: {data.meta.monitoringMode} | Role: {data.permissions.role} | Selected: {selected.symbol} | Updated: {time(data.meta.timestamp)}</p>
        </div>
        <div className="hidden flex-wrap justify-end gap-2 sm:flex">
          <Button variant="outline" disabled={!data.permissions.canRefresh || query.action.isPending} onClick={() => command("Refresh quote snapshot", "/api/mt5/market-watch/refresh")}><RefreshCw className="h-4 w-4" />Refresh Quotes</Button>
          <Button variant="outline" disabled={!data.permissions.canDiagnostics || query.action.isPending} onClick={() => command("Run market diagnostics", "/api/mt5/market-watch/diagnostics")}><Bot className="h-4 w-4" />Diagnostics</Button>
          <Button variant="outline" disabled={!data.permissions.canManageWatchlist || query.action.isPending} onClick={() => command(selected.watchlisted ? "Remove selected from watchlist" : "Add selected to watchlist", `/api/mt5/market-watch/instruments/${selected.id}/watchlist`)}><Star className="h-4 w-4" />{selected.watchlisted ? "Unwatch Selected" : "Watch Selected"}</Button>
          <Button variant="outline" onClick={exportReport}><Download className="h-4 w-4" />Export Report</Button>
        </div>
        <div className="sm:hidden"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline"><Menu className="h-4 w-4" />Actions</Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem disabled={!data.permissions.canRefresh} onSelect={() => command("Refresh quote snapshot", "/api/mt5/market-watch/refresh")}>Refresh Quotes</DropdownMenuItem><DropdownMenuItem disabled={!data.permissions.canDiagnostics} onSelect={() => command("Run market diagnostics", "/api/mt5/market-watch/diagnostics")}>Run Diagnostics</DropdownMenuItem><DropdownMenuItem disabled={!data.permissions.canManageWatchlist} onSelect={() => command("Toggle watchlist", `/api/mt5/market-watch/instruments/${selected.id}/watchlist`)}>Toggle Watchlist</DropdownMenuItem><DropdownMenuItem onSelect={exportReport}>Export Report</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div>
      </div>
