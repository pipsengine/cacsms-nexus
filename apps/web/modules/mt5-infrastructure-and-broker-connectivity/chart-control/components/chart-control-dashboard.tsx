...
export function ChartControlDashboard() {
  ...
  const analysis = selected ? data.analysisByInstrument[selected.id] : undefined;
  const series = selected ? visibleCandles(selected.candles, selected.timeframe) : [];
  ...
  const selectedDrawings = selected ? data.drawings.filter((drawing) => drawing.instrumentId === selected.id && drawing.visible) : [];
  ...
  return <div className="mx-auto flex w-full max-w-[1900px] flex-col gap-4 px-4 py-5 sm:px-6 lg:px-8">
    ...
    <section className="grid gap-4 xl:grid-cols-[1.55fr_0.45fr]">
      <Card>
        ...
        <CardContent>
          {!selected || !analysis ? <p className="rounded-xl border border-dashed border-slate-200 p-8 text-center text-sm text-slate-600">No chart instruments yet. Connect NexusBridgeEA and attach it to a chart symbol to populate live chart feeds.</p> : <>
    ...
    <section className="grid gap-4 xl:grid-cols-[1fr_0.75fr_0.75fr]">
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">{selected && series.length ? <><div className="h-48"><ResponsiveContainer width="100%" height="100%"><BarChart data={series}><XAxis dataKey="timestamp" tickFormatter={time} tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 10 }} /><ChartTooltip /><Bar dataKey="volume" fill="#2563eb" name="Volume" /></BarChart></ResponsiveContainer></div><div className="h-48"><ResponsiveContainer width="100%" height="100%"><AreaChart data={series}><XAxis dataKey="timestamp" tickFormatter={time} tick={{ fontSize: 9 }} /><YAxis domain={[0, 100]} tick={{ fontSize: 10 }} /><ChartTooltip /><ReferenceLine y={70} stroke="#dc2626" strokeDasharray="4 3" /><ReferenceLine y={30} stroke="#10b981" strokeDasharray="4 3" /><Area dataKey="rsi" stroke="#7c3aed" fill="#ede9fe" name="RSI 14" /></AreaChart></ResponsiveContainer></div></> : <p className="text-sm text-slate-600 md:col-span-2">Volume and momentum panels appear when a live chart instrument is available.</p>}</CardContent>
      </Card>
      <Card>
