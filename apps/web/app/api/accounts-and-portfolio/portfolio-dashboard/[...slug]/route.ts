  ...
  try {
    if (slug[0] === "events-stream") {
      return createMt5EventStream({
        request,
        eventName: "portfolio-dashboard-snapshot",
        snapshot: () => buildPortfolioDashboardResponse(role, accountId)
      });
    }
    throw new Error("not found");
  } catch (error) {
    if (error instanceof Error && error.message === "not found") {
      return failure(new Error("Portfolio dashboard endpoint not found."));
    }
