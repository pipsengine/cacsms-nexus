...
import { buildSymbolSyncResponse } from "@/app/api/mt5/symbol-sync/_lib/store";
...
vi.mock("@/modules/mt5-infrastructure-and-broker-connectivity/symbol-sync/hooks/use-symbol-sync", () => ({
  useSymbolSync: () => ({ data: buildSymbolSyncResponse("Read-Only Viewer"), isLoading: false, isError: false, streamConnected: true, refetch: vi.fn(), action: { mutateAsync: vi.fn(), isPending: false } })
}));
afterEach(cleanup);
