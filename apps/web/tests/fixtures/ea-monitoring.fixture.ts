import type { AuditRecord, ScoreResult } from "@/modules/mt5-infrastructure-and-broker-connectivity/mt5-control-center/types/mt5-control-center.types";
import type {
  AiEaDiagnostic,
  EaAnalyticsPoint,
  EaCommand,
...
export function createEaMonitoringSeed() {
  ...
  const kpis: EaKpi[] = [
    ...
    { label: "Highest Risk EA", value: highest?.eaId ?? "G��", status: "Critical", detail: highest?.lastError ?? "Highest risk based on readiness and failures.", updatedAt: isoNow() },
